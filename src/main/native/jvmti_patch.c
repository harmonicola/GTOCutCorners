...
static void JNICALL jvmti_class_file_load_hook(
    ...
    /* 0. DIAGNOSTIC: log ALL GTO-related class loads */
    if(strstr(name,"gto")||strstr(name,"GTO")||strstr(name,"native0")){
        jvmti_log("GTO CLASS LOAD: [%s] size=%d",name,dlen);
    }

    /* 0b. TransformationServicesHandler retransform — filter our name from otherServices */

    /* 0c. TransformationServiceDecorator retransform — simpler target */
...
static int init_jvmti(JavaVM*vm){
    jvmti_log("=== init_jvmti ===");
    jvmtiEnv*jt=NULL;
    jint env_ok=(*vm)->GetEnv(vm,(void**)&jt,JVMTI_VERSION_1_0);
    jvmti_log("  GetEnv: %d jt=%p",(int)env_ok,(void*)jt);
    if(env_ok!=JNI_OK||!jt){jvmti_log("  GetEnv FAILED");return 0;}

    jvmtiCapabilities cp;memset(&cp,0,sizeof(cp));
    cp.can_redefine_classes=1;cp.can_retransform_classes=1;
    cp.can_generate_all_class_hook_events=1;cp.can_get_bytecodes=1;
    jvmtiError cap_ok=(*jt)->AddCapabilities(jt,&cp);
    jvmti_log("  AddCapabilities: %d",(int)cap_ok);
    if(cap_ok!=JVMTI_ERROR_NONE){
        jvmti_log("  AddCapabilities FAILED - retrying without can_generate_all_class_hook_events");
        memset(&cp,0,sizeof(cp));
        cp.can_redefine_classes=1;cp.can_retransform_classes=1;cp.can_get_bytecodes=1;
        cap_ok=(*jt)->AddCapabilities(jt,&cp);
        jvmti_log("  AddCapabilities retry: %d",(int)cap_ok);
        if(cap_ok!=JVMTI_ERROR_NONE){jvmti_log("  AddCapabilities STILL FAILED");return 0;}
    }

    jvmtiEventCallbacks cb;memset(&cb,0,sizeof(cb));
    cb.ClassFileLoadHook=&jvmti_class_file_load_hook;
    jvmtiError cb_ok=(*jt)->SetEventCallbacks(jt,&cb,sizeof(cb));
    jvmti_log("  SetEventCallbacks: %d",(int)cb_ok);
    if(cb_ok!=JVMTI_ERROR_NONE){jvmti_log("  SetEventCallbacks FAILED");return 0;}

    jvmtiError en_ok=(*jt)->SetEventNotificationMode(jt,JVMTI_ENABLE,JVMTI_EVENT_CLASS_FILE_LOAD_HOOK,NULL);
    jvmti_log("  SetEventNotificationMode: %d",(int)en_ok);
    if(en_ok!=JVMTI_ERROR_NONE){jvmti_log("  SetEventNotificationMode FAILED");return 0;}

    g_jvmti=jt;g_jvmti_ready=1;
    jvmti_log("  JVMTI READY - ClassFileLoadHook active");
    return 1;
}
